...
public class Matrix
{
    ...
    public Matrix(Vector3 pos, Quaternion rotation)
    {
        Matrix subRotMat = new Matrix(rotation);
        mat = MatrixCreate(4, 4);
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                this[i,j] = subRotMat[i, j];
            }
        }
    ...
    public Vector3 MultiplyPoint(Vector3 pos)
    {
        if (rows != 4 || cols != 4)
        {
            throw new Exception("Matrix is not 4x4, cannot multiply point, matrix is " + rows + "x" + cols + " instead");
        }

        Matrix posMat = new Matrix(4, 1);
        posMat[0, 0] = pos.x;
        posMat[1, 0] = pos.y;
        posMat[2, 0] = pos.z;
        posMat[3, 0] = 1;
        Matrix res = this * posMat;
